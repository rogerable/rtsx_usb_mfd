...
static int sd_read_data(struct rtsx_usb_sdmmc *host, u8 *cmd, u16 byte_cnt,
		u8 *buf, int buf_len, int timeout)
{
	...
	for (i = 0; i < 5; i++)
		rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, SD_CMD0 + i, 0xFF, cmd[i]);
	...
	err = rtsx_usb_get_rsp(ucr, 1, timeout);
	...
	if (buf && buf_len) {
		err = rtsx_usb_read_ppbuf(ucr, buf, buf_len);
		if (err) {
			dev_dbg(sdmmc_dev(host),
...
static int sd_write_data(struct rtsx_usb_sdmmc *host, u8 *cmd, u16 byte_cnt,
		u8 *buf, int buf_len, int timeout)
{
	struct rtsx_ucr *ucr = host->ucr;
	int err, i;
...
static int sd_pull_ctl_disable(struct rtsx_ucr *ucr)
{
	rtsx_usb_init_cmd(ucr);
...
static int sd_pull_ctl_enable(struct rtsx_ucr *ucr)
{
	rtsx_usb_init_cmd(ucr);
...
static int sd_power_off(struct rtsx_usb_sdmmc *host)
{
	...
	return sd_pull_ctl_disable(ucr);
}
...
#ifdef CONFIG_PM
static int rtsx_usb_sdmmc_suspend(struct platform_device *pdev,
		pm_message_t state)
{
...
static int rtsx_usb_sdmmc_resume(struct platform_device *pdev)
{
	struct rtsx_usb_sdmmc *host = platform_get_drvdata(pdev);
	struct mmc_host *mmc = host->mmc;
...
#else /* CONFIG_PM */
#define rtsx_usb_sdmmc_suspend NULL
#define rtsx_usb_sdmmc_resume NULL
#endif /* CONFIG_PM */
